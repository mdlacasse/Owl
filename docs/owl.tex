...
\item [$d_{in}$]
	...
	These deposits are coming from the surplus $s_n$, distributed to each spousal account
	depending on parameter $\eta$.
\item [$f_{t n}$]
...
\item [$s_{n}$]
	Surplus of funds during year $n$, most likely caused by RMDs or influx of money from
	big-ticket items (inheritance, gifts, sale of a house, etc.).
\item [$w_{ijn}$]
...
\item [$\eta$]
	...
	that goes to the $i = 1$ second spouse's account. Therefore,
	\begin{eqnarray}
		\label{Eq:eta}
		d_{0n} & = & (1 - \eta) s_n \nonumber\\
		d_{1n} & = & \eta s_n.
	\end{eqnarray}
	This choice is such that we can set a value depending on individual $\eta_i = \delta(i, i_s)$
	for $n \ge n_d$, after the passing of $i_d$.
\item [$\mathcal{M}_n$]
...
\paragraph*{Required minimum distributions (RMDs)}
	...
	or equal than the required minimum distributions, and therefore, $\forall i$,
	\begin{equation}
		\label{Eq:C1}
...
\paragraph*{Account balances}
	...
	between surplus, withdrawals, and conversions:
	\begin{eqnarray*}
		s_n &\texttt{XOR} & w_{ijn}, \\
	...
	is to move all transactions to be synchronous at the beginning or the end of the year
	...
	\begin{eqnarray}
		...
		b_{ij0} = \beta_{ij}.
	\end{eqnarray}
	At this point, we assume that the accounts are balanced according to the desired
	allocation ratios $\alpha_{ijkn}$.

	We also introduce another constraint that might look unnecessary, but helps
	convergence. As the withdrawals are at the beginning of the year 
	we impose that
	\begin{eqnarray}
		w_{ijn} \le b_{ijn}.
	\end{eqnarray}
...
\paragraph*{Net spending}
	...
	Then we subtract potential surplus $s_{n}$ and all taxes paid:
	\begin{eqnarray}
		g_n = \sum_i [\omega_{in} + \bar{\zeta}_{in} + \pi_{in} ] 
		+ \sum_{ij} w_{ijn} + \sum_i \Lambda^\pm_{in} - s_{n}
		- T_n - U_n - \mathcal{M}_n.
	\end{eqnarray}
	For spouses, the surplus $s_n$ get deposited to the taxable accounts
