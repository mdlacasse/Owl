...
\item [$T_n$]
	...
	As the product $\bar{\Delta}_{tn}\theta_{tn}$ does not guarrantee to
...
\item [$I_n$]
	Income Related Monthly Adjusted Adjustment for Medicare. As this additional adjustment
	is a step function, it has to be computed using binary variables. The adjustment
	is computed from the modified adjusted gross income (MAGI) from 2 years earlier. For the
	MAGI, we will simply use $G_{n-2} + \bar{\sigma}_{n-2}$ and ignore the additional IRS
	rules around tax-free interests.

	There are $q=5$ levels
	of step adjustments, $\bar{L}_{qn} = L_q\gamma_n$ and each of them introduce an annual additional medicare
	cost of $\bar{C}_{qn} = C_q\gamma_n$.  Both levels and costs are adjusted for inflation.
	Using binary variables $z_{inq}$ and the following constraint
	\begin{equation}
		\bar{\sigma}_{n-2} -\bar{L}_{qn} \le z_{inq} M - G_{n-2} \le bigM - \bar{L}_{qn} + \bar{\sigma}_{n-2},
	\end{equation}
	the IRMAA adjustements can then computed as
	\begin{equation}
		\label{Eq:IRMAA}
		I_n = \sum_{iq} z_{iqn} \bar{C}_{qn}.
	\end{equation}
...
\paragraph*{Account balances}
	...
	a path to shelter negative returns by performing an overwithdrawal from the taxable
	...
	We also introduce another contraint that might look unnecessary, but helps
...
\paragraph*{Net spending}
	...
	\begin{eqnarray}
		\label{Eq:C4}
		g_n + I_n - \sum_{ij} w_{ijn} + \sum_{i}d_{in}
	...
	For clarity, we did not expand $I_n$ in terms of the binary variables $z_{inq}$,
	and will leave it to the solver.
...
\begin{equation}
	...
	+ \Lambda^\pm_{in} - .5\psi\mu\alpha_{i00n}\kappa_{i0n}].
\end{equation}
