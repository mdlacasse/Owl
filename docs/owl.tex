...
\item [$M_n$]
	Medicare and Income Related Monthly Adjusted Adjustment for Medicare. As this additional adjustment
	is a step function, it has to be computed using binary variables and mixed-integer linear
	programming. The adjustment
...
\paragraph*{Net spending}
	...
	\begin{eqnarray}
		...
		&& \sum_i [\Lambda^\pm_{in} - .5\psi\mu\alpha_{i00n}\kappa_{i0n}] - M_n^s.
	\end{eqnarray}
	...
	For clarity, we did not express $M_n^s(G_n+\bar{sigma}_n)$ in terms of the modified
...
\begin{equation}
	...
	+ \Lambda^\pm_{in} - .5\psi\mu\alpha_{i00n}\kappa_{i0n}] - M_n^s.
\end{equation}
