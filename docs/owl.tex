...
\item [$G_n$]
	...
	deposits and withdrawals are taking place at the beginning of the year. 
...
\item [$T_n$]
	...
	As the product $\bar{\Delta}_{tn}\theta_{tn}$ does not garrantee to
...
\paragraph*{Account balances}
	...
	make good sense. Conversions are also typically best if they are
...
\begin{eqnarray}
	...
	A_e[J_0(i, j, n), q_x(i, n)] &=& -(\delta(j, 2) - \delta(j, 1))(1 + \mathcal{T}{ijn}), \nonumber \\
...
\begin{eqnarray}
	A_e[J_3(n), q_g(n)] &=& 1, \nonumber \\
	A_e[J_3(n), q_w(i, j ,n)] &=& -1,\nonumber \\
	A_e[J_3(n), q_d(i, n)] &=& 1 + \psi\mu\alpha_{i00n}, \nonumber \\
	A_e[J_3(n), q_f(t, n)] &=& \bar\Delta_{t n}\theta_{t n}, \nonumber \\
	A_e[J_3(n), q_b(i, 0, n)] &=& \psi\mu\alpha_{i00n}, \nonumber \\
	A_e[J_3(n), q_w(i, 0, n)] &=& \psi\alpha_{i00n}(\max(0, \tau_{0n-1} - \mu), \nonumber \\
	&&\qquad\forall t \in \{0,\ldots, N_t-1\},\nonumber\\
	&&\qquad\forall i \in \{0,\ldots, N_i-1\},\nonumber\\
	&&\qquad\forall j \in \{0,\ldots, N_j-1\},\nonumber\\
	&&\qquad\forall n \in \{0,\ldots, N_n-1\}, \nonumber
...
\begin{eqnarray}
	...
	A_e[J_5(n), q_b(i, 0, n)] &=&  (1 - \delta(k, 0))\alpha_{i0kn}\tau_{kn}, \\
	A_e[J_5(n), q_w(i, 0, n)] &=& -(1 - \delta(k, 0))\alpha_{i0kn}\tau_{kn}, \nonumber \\
	A_e[J_5(n), q_d(i, n)] &=&     (1 - \delta(k, 0))\alpha_{i0kn}\tau_{kn}, \nonumber \\
	A_e[J_5(n), q_x(i, n)] &=& -1, \nonumber \\
	&&\qquad\forall t \in \{0,\ldots, N_t-1\},\nonumber\\
	&&\qquad\forall i \in \{0,\ldots, N_i-1\},\nonumber\\
...
\begin{eqnarray}
	...
	&&\times \Bigl\{\left[b_{ijn} + (\delta(j, 2) - \delta(j, 1))x_{in}
	+ .5\kappa_{ijn}\right](1 + \tau_{kn}) 
	\nonumber \\
	&& + .5 \kappa_{ijn} - w_{ijn} + \delta(j, 0)d_{in} \Bigr\}
	+ (\phi_j\delta(n, n_d-1)\delta(i, i_s)) \nonumber  \\
	&& \times \Bigl\{\left[b_{i_djn} + (\delta(j, 2) - \delta(j, 1))x_{i_dn}
	+ .5\kappa_{i_djkn}\right](1 + \tau_{kn}) 
	\nonumber \\
	&& + .5 \kappa_{i_djn} - w_{i_djn} + \delta(j, 0)d_{i_dn}\Bigr\} .
\end{eqnarray}
