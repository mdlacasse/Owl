...
\paragraph*{Account balances}
	...
	by introducing binary variables $z_{in}^R \in \{0, 1\}$ with the following constraints:
	\begin{alignat}{2}
		\label{Eq:Binary}
		0 & \le \; z_{in}^R \mathcal{M} - x_{in} &\le \mathcal{M}, \nonumber \\
		0 & \le \; z_{in}^R \mathcal{M} + w_{i2n} &\le \mathcal{M}.
	\end{alignat}
...
\paragraph*{Roth Conversions}
	...
	\end{equation}
	This last equation will typically need to be coded as two separate constraints.
...
\paragraph*{Taxable ordinary income}
	...
	Finally, the standard exemption is constrained by
	\begin{equation}
...
Adding binary variables $z^R_{inz}$ can adds $N_zN_iN_n$ more decision variables,
...
\end{equation}
Here we used the fact that the sum of all deposits $d_{in}$ must equal the surplus $s_n$,
\[
	s_n = \sum_i d_{in}.
\]
...
Only the deposit scheme has been implemented.
