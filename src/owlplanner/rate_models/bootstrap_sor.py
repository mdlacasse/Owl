...
class BootstrapSORRateModel(BaseRateModel):
    ...
    description = (
        "Advanced bootstrap sequence-of-returns model with IID, block, "
        "circular, stationary bootstrap, and optional crisis overweighting."
    )

    required_parameters = {
        "frm": "First historical year (inclusive)",
        "to": "Last historical year (inclusive)",
    }

    optional_parameters = {
        "bootstrap_type": "iid | block | circular | stationary (default=iid)",
        "block_size": "Block length (default=1)",
        "crisis_years": "List of years to overweight",
        "crisis_weight": "Multiplier for crisis sampling probability (default=1.0)",
    }
