...
\item [$q_{pn}$]
	...
	The bracket widths are enforced by $q_{0n} + e_n \le T^{15}_n$ and
	$q_{0n} + q_{1n} + e_n \le T^{20}_n$, where $T^{15}_n$ and $T^{20}_n$ are the
	inflation-adjusted MAGI thresholds above which the 15\% and 20\% rates apply.
	Since the cost function is strictly convex ($0 < 0.15 < 0.20$), no binary variables
	are required: the LP naturally fills the lowest-cost bracket first.
\item [$t^{\sigma}_n$]
...
\paragraph*{LTCG bracket constraints}
	...
	The bracket upper-bound constraints
	\begin{eqnarray}
		\label{Eq:LTCG2}
		q_{0n} + e_n &\le& T^{15}_n, \\
		q_{0n} + q_{1n} + e_n &\le& T^{20}_n \label{Eq:LTCG3}
	\end{eqnarray}
	prevent overflow into higher-rate brackets.
	Here $T^{15}_n$ and $T^{20}_n$ are the inflation-adjusted MAGI thresholds above which
	the 15\% and 20\% LTCG rates apply, and $e_n$ is the ordinary taxable income that
	already occupies the lower part of the income range.
