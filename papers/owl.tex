...
\item [$q_{pn}$]
	...
	\begin{eqnarray}
		...
		q_{0n} &\le& \max\!\left(0,\; T^{15}_n - G_n\right), \label{Eq:q0cap} \\
		q_{0n} + q_{1n} &\le& \max\!\left(0,\; T^{20}_n - G_n\right), \label{Eq:q1cap}
	\end{eqnarray}
	where $T^{15}_n$ and $T^{20}_n$ are the inflation-adjusted total taxable income
...
\item [$T^{15}_n$, $T^{20}_n$]
	Inflation-adjusted total taxable income thresholds above which the 15\% and 20\%
	long-term capital gains (LTCG) tax rates apply, respectively.
	LTCG is taxed at 0\% when total taxable income (ordinary + LTCG) is below $T^{15}_n$,
	at 15\% between $T^{15}_n$ and $T^{20}_n$, and at 20\% above $T^{20}_n$.
	In 2026, $(T^{15}, T^{20}) = (\$49{,}450,\;\$545{,}500)$ for single filers and
...
\item [$\mathcal{T}_n$]
	...
	$\mathcal{T}_n$ is computed from the IRS formula in Eqs.~(\ref{Eq:TaxableSS})--(\ref{Eq:TaxableSSSimpl}),
	and $\Psi_n = \mathcal{T}_n / \sum_i\bar{\zeta}_{in}$.
...
\paragraph*{LTCG bracket constraints}
	...
	\begin{eqnarray}
		\label{Eq:LTCG2}
		q_{0n} &\le& \max(0,\; T^{15}_n - G_n), \\
		q_{0n} + q_{1n} &\le& \max(0,\; T^{20}_n - G_n), \label{Eq:LTCG3}
	\end{eqnarray}
	where $T^{15}_n$ and $T^{20}_n$ are the inflation-adjusted total taxable income
...
\begin{eqnarray}
	...
	&&\times \Bigl\{\left[b_{ijn} + \delta_{j, 0}d_{ij} - w_{ijn}
...
\begin{eqnarray}
	...
	+ (\delta_{j, 2} - \delta_{j, 1})x_{ikn} \right](1 + \mathcal{T}_{ijn}) \Bigr\}
	\nonumber \\
	&&- [\phi_j\delta_{n, n_d-1}\delta_{i, i_s}] \nonumber \\
	&& \times \Bigl\{ \left[b_{i_djn} + \delta_{j, 0}d_{in} - w_{ijn}
	+ (\delta_{j, 2} - \delta_{j, 1})x_{i_dkn}
	\right](1 + \mathcal{T}_{i_djn})
	\Bigr\}
	\nonumber \\
...
by the historical, histochastic, bootstrap, and VAR(1) models.
...
calendar year $y$ by $r_{k,y}$.

\section{Fixed rate methods}
...
\begin{description}[leftmargin=4em,style=multiline]
\item [\texttt{default}]
    Trailing 30-year historical averages, updated annually.
...
\item [\texttt{historical average}]
    The arithmetic mean over the selected historical window,
    \begin{equation}
        \tau_k = \frac{1}{N_w}\sum_{y=\mathcal{Y}_{\mathrm{frm}}}^{\mathcal{Y}_{\mathrm{to}}}
                 r_{k,y}.
    \end{equation}
\item [\texttt{user}]
...
\subsection{Multivariate normal: \texttt{histochastic} and \texttt{gaussian}}
...
The method name \texttt{stochastic} is retained as a backward-compatible
alias for \texttt{gaussian}; existing case files continue to work unchanged.
...
total-loss artefacts are possible), produces a right-skewed marginal
...
parameters.
Let $\ell_{k,y} = \ln(1 + r_{k,y})$ be the log-return of asset class $k$
...
\begin{eqnarray}
    ...
    (\bm{\Sigma}_Z)_{kk'} &=& \frac{1}{N_w-1}
                  \sum_{y=\mathcal{Y}_{\mathrm{frm}}}^{\mathcal{Y}_{\mathrm{to}}}
                  (\ell_{k,y} - \mu_{Z,k})(\ell_{k',y} - \mu_{Z,k'}).
\end{eqnarray}
...
model of the \texttt{histochastic} method.
...
Let $\varepsilon_{k,t} = y_{k,t} - \mu_k$ be the demeaned return of
asset class $k$ at year $t$.  The conditional variance $h_{k,t}$
evolves as
\begin{equation}
    \label{Eq:GARCH11}
    h_{k,t} = \omega_k + \alpha_k\,\varepsilon_{k,t-1}^2
              + \beta_k\,h_{k,t-1},
\end{equation}
with $h_{k,0} = \mathrm{Var}(\varepsilon_k)$.
The parameters $(\omega_k, \alpha_k, \beta_k)$ are estimated
independently for each asset by maximising the Gaussian
log-likelihood
\begin{equation}
    \ell_k = -\tfrac{1}{2}\sum_{t=1}^{N_w}
             \bigl[\ln h_{k,t} + \varepsilon_{k,t}^2/h_{k,t}\bigr],
\end{equation}
subject to $\omega_k > 0$, $\alpha_k, \beta_k > 0$, and the
covariance-stationarity constraint $\alpha_k + \beta_k < 1$.
Optimisation uses L-BFGS-B; if convergence fails, an EWMA
fallback ($\alpha_k = 0.06$, $\beta_k = 0.94$) is used instead.

The standardised residuals $z_{k,t} = \varepsilon_{k,t}/\sqrt{h_{k,t}}$
are passed to the second step.

\paragraph*{Step 2 --- DCC dynamics}
Let $\overline{\mathbf{Q}} = \frac{1}{N_w}\mathbf{Z}^\top\mathbf{Z}$
be the unconditional covariance of $\mathbf{Z} = [z_{k,t}]$.
...
is obtained by normalising:
\begin{equation}
    \mathbf{R}_t = \mathrm{diag}(\mathbf{Q}_t)^{-1/2}\,
                  \mathbf{Q}_t\,
                  \mathrm{diag}(\mathbf{Q}_t)^{-1/2}.
\end{equation}
The DCC parameters $(a, b)$ are estimated by maximising the
...
(\texttt{histochastic}, \texttt{histolognormal}, \texttt{gaussian},
...
\texttt{http://www.econ.yale.edu/\textasciitilde shiller/data.htm}.
