...
\item [$\Psi$]
	...
	given how low the threshold (around \$44k for married couples) is for reaching the
...
\item [$\mathbb{I}_n$]
	...
	\end{equation}
	Note that $\mathbb{I}$ can be negative by definition, and will need to be clamped to zero if so.
...
\item [$J_n$]
	...
	$G^{\mathrm{NIIT}}$ (not indexed for inflation). We compute
	\begin{equation}
		J_n = 0.038\ \max\!\left(0,\ \min\!\left(\mathbb{G}_n - G^{\mathrm{NIIT}},\ \mathbb{I}_n + Q_n\right)\right),
	\end{equation}
	where $G^{\mathrm{NIIT}}$ is \$200k for single filers and \$250k for married filing jointly,
...
\paragraph*{Medicare brackets and costs}
	...
	To preserve the monotonic structure of the IRMAA brackets, we also impose
	\begin{equation}
		\label{Eq:MedMono}
		z_{(q+1)n}^m \le z_{qn}^m, \qquad q \in \{0, \ldots, N_q-3\},
	\end{equation}
	so that activating a higher bracket implies all lower brackets are active as well.
...
\begin{eqnarray}
	A_u[I_7(i, n), q_{x}(i, n)] &=& 1, \nonumber \\
	A_u[I_7(i, n), q_{b}(i, 1, n)] &=& -1, \nonumber \\
	u[I_7(i, n)] &=& 0, \nonumber \\
	&&\qquad\forall i \in \{0,\ldots, N_i - 1\},\nonumber\\
	&&\qquad\forall n \in \{0,\ldots, N_n - 1\}. \nonumber
\end{eqnarray}
...
\begin{eqnarray}
	A_u[I_{15}(q, n), q_{w}(i, 1, n-2)] &=& -1, \nonumber \\
	A_u[I_{15}(q, n), q_{x}(i, n-2)] &=& -1, \nonumber \\
	A_u[I_{15}(q, n), q_{b}(i, 0, n-2)] &=&
	    - \mu \alpha_{i00(n-2)} -\sum_{k\neq 0} \alpha_{i0k(n-2)}\tau_{k(n-2)},
	    \nonumber \\
	A_u[I_{15}(q, n), q_{d}(i, n-2)] &=&
	    - \mu \alpha_{i00(n-2)} -\sum_{k\neq 0} \alpha_{i0k(n-2)}\tau_{k(n-2)},
	    \nonumber \\
	A_u[I_{15}(q, n), q_{w}(i, 0, n-2)] &=&
	    \mu \alpha_{i00(n-2)} + \sum_{k\neq 0} \alpha_{i0k(n-2)}\tau_{k(n-2)}
	    \nonumber \\
	    && - \alpha_{i00(n-2)}\max(0, \tau_{0\max(0, n-3)} - \mu), \nonumber \\
	A_u[I_{15}(q, n), q_{z^m}(q, n)] &=& \mathcal{M}, \nonumber \\
	u[I_{15}(q, n)] &=& -\bar{\mathcal{L}}_{qn} + \mathcal{M}
...
\end{eqnarray}
To enforce the monotonicity of the bracket indicators, we add
\begin{eqnarray}
	A_u[I_{19}(q, n), q_{z^m}(q+1, n)] &=& 1, \nonumber \\
	A_u[I_{19}(q, n), q_{z^m}(q, n)] &=& -1, \nonumber \\
	u[I_{19}(q, n)] &=& 0, \nonumber \\
	&&\qquad\forall q \in \{0,\ldots, N_q - 3\}, \nonumber\\
	&&\qquad\forall n \in \{\max(n_m, 0),\ldots, N_n - 1\}.
\end{eqnarray}
When $n_m < 2$, we use these additional rows
\begin{eqnarray}
	A_u[I_{16}(q, n), q_{z^m}(q, n)] &=& \mathcal{M}, \nonumber \\
	u[I_{16}(q, n)] &=& \mathcal{M} + \mathcal{A}_{n} -\bar{\mathcal{L}}_{qn}, \nonumber \\
	&&\qquad\forall q \in \{0,\ldots, N_q - 2\}, \nonumber\\
...
\begin{eqnarray}
	A_u[I_{17}(q, n), q_{w}(i, 1, n-2)] &=& 1, \nonumber \\
	A_u[I_{17}(q, n), q_{x}(i, n-2)] &=& 1, \nonumber \\
	A_u[I_{17}(q, n), q_{b}(i, 0, n-2)] &=&
	    \mu \alpha_{i00(n-2)} + \sum_{k\neq 0} \alpha_{i0k(n-2)}\tau_{k(n-2)},
	    \nonumber \\
	A_u[I_{17}(q, n), q_{d}(i, n-2)] &=&
	    \mu \alpha_{i00(n-2)} + \sum_{k\neq 0} \alpha_{i0k(n-2)}\tau_{k(n-2)},
	    \nonumber \\
	A_u[I_{17}(q, n), q_{w}(i, 0, n-2)] &=&
	    - \mu \alpha_{i00(n-2)} -\sum_{k\neq 0} \alpha_{i0k(n-2)}\tau_{k(n-2)}
	    \nonumber \\
	    && + \alpha_{i00(n-2)}\max(0, \tau_{0\max(0, n-3)} - \mu), \nonumber \\
	A_u[I_{17}(q, n), q_{z^m}(q, n)] &=& -\mathcal{M}, \nonumber \\
	u[I_{17}(q, n)] &=& \bar{\mathcal{L}}_{qn}
         - \sum_i \left[\omega_{i(n-2)} + \bar{\zeta}_{i(n-2)} + \bar{\pi}_{i(n-2)} \right]\nonumber \\
	&& - 0.5 \sum_i \left[\kappa_{i0(n-2)}  \left(
	  \mu\alpha_{i00(n-2)} + \sum_{k\neq 0} \alpha_{i0k(n-2)}\tau_{k(n-2)}
	  \right)\right],\nonumber\\
...
\begin{eqnarray}
	A_u[I_{18}(q, n), q_{z^m}(q, n)] &=& -\mathcal{M}, \nonumber \\
	u[I_{18}(q, n)] &=& \bar{\mathcal{L}}_{qn} - \mathcal{A}_{n} , \nonumber \\
	&&\qquad\forall q \in \{0,\ldots, N_q - 2\}, \nonumber\\
	&&\qquad\forall n \in \{n_m,\ldots, 2\},
\end{eqnarray}
where $\mathcal{A}_n$ is a user-provided array of MAGI values for (up to) the last two years in chronological order.
...
\begin{eqnarray}
	A_u[J_2(n), q_{m}(n)] &=& 1, \nonumber \\
	A_u[J_2(n), q_{z^m}(q, n)] &=& -(\bar{\mathcal{C}}_{(q+1)n} - \bar{\mathcal{C}}_{qn}), \nonumber \\
	u[J_2(n)] &=& \bar{\mathcal{C}}_{0n},\\
	&&\qquad\forall q \in \{0,\ldots, N_q - 2\}, \nonumber\\
...
\begin{eqnarray}
	...
	A_e[J_3(n), q_m(n)] &=& 1, \nonumber \\
	A_e[J_3(n), q_s(n)] &=& 1, \nonumber \\
	A_e[J_3(n), q_b(i, 0, n)] &=& \psi_n\mu\alpha_{i00n}, \nonumber \\
	A_e[J_3(n), q_d(i, n)] &=& \psi_n\mu\alpha_{i00n}, \nonumber \\
	A_e[J_3(n), q_{f}(t, n)] &=& \theta^x_{tn}, \nonumber \\
	A_e[J_3(n), q_w(i, j ,n)] &=& -1 + 0.1(1-\delta(j, 0))(1-\mathcal{H}(n-n_{i, 59}))
...
where $v$ is
\begin{equation}
	v[J_3(n)] = \sum_{*=x,c,f} \mathcal{A}^*_n + \sum_i \Lambda^\pm_{in}
...
\begin{eqnarray}
	A_e[J_5(n), q_e(n)] &=& 1, \nonumber \\
	A_e[J_5(n), q_{f}(t, n)] &=& 1, \nonumber \\
	A_e[J_5(n), q_w(i, 1, n)] &=& -1, \nonumber \\
	A_e[J_5(n), q_x(i, n)] &=& -1, \nonumber \\
	A_e[J_5(n), q_b(i, 0, n)] &=& -\sum_{k\neq 0} \alpha_{i0kn}\tau_{kn}, \\
	A_e[J_5(n), q_w(i, 0, n)] &=&  \sum_{k\neq 0} \alpha_{i0kn}\tau_{kn}, \nonumber \\
	A_e[J_5(n), q_d(i, n)] &=&    -\sum_{k\neq 0} \alpha_{i0kn}\tau_{kn}, \nonumber \\
	&&\qquad\forall t \in \{0,\ldots, N_t-1\},\nonumber\\
	&&\qquad\forall i \in \{0,\ldots, N_i-1\},\nonumber\\
	&&\qquad\forall k \in \{0,\ldots, N_k-1\},\nonumber\\
	&&\qquad\forall n \in \{0,\ldots, N_n-1\}, \nonumber
