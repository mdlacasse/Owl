...
\paragraph*{Medicare brackets and costs}
	...
	higher $q$ values include the IRMAA additional costs.
        ...
        To model the IRMAA brackets without big-$\mathcal{M}$, we introduce
	binary variables $z_{qn}^m \in \{0, 1\}$ (one per bracket) and continuous
	variables $h_{qn} \ge 0$ for capturing that the MAGI is falling within bracket $q$.
	...
	\begin{eqnarray}
		...
		\bar{\mathcal{L}}_{(N_q-2)n} z_{(N_q-1)n}^m &\le& h_{(N_q-1)n}, \nonumber
	\end{eqnarray}
	...
	\end{equation}
	as each $z_{qn}^m$ selects a unique bracket.
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	For plans where $n_m < 2$, we use the user-provided MAGI values $\mathbb{G}_{(-1)}$ and possibly
...
\begin{eqnarray}
	...
	A_e[J_5(n), q_w(i, j ,n)] &=& -1 + 0.1(1-\delta(j, 0))(1-\mathcal{H}(n-n_{i, 59}))
