...
\item [$p_{rn}$]
	Provisional income in Social Security taxability zone $r \in \{0, 1, 2\}$ during year $n$.
	Zone 0 holds PI below the lower statutory threshold $\mathcal{P}^{\mathrm{lo}}_n$ (no Social Security taxable);
	zone 1 holds PI between the two thresholds (50\% of the zone-1 amount is taxable);
	zone 2 holds PI above the upper threshold $\mathcal{P}^{\mathrm{hi}}_n$ (85\% of the zone-2 amount is taxable).
	The decomposition $\Pi_n = \sum_r p_{rn}$ connects these tier variables to the
	provisional income, which is a linear function of the existing LP decision variables.
	See the Social Security taxability paragraph in the Constraints section for the complete formulation.
\item [$m_n$]
...
\item [$\Psi_n$]
	...
	This quantity depends on provisional income and is bounded by 85\%.
	In the current LP formulation $\Psi_n$ is not an input parameter: it is determined
	endogenously by the provisional income tier variables $p_{rn}$ as
	$\Psi_n = (0.5\,p_{1n} + 0.85\,p_{2n})\,/\!\sum_i\bar{\zeta}_{in}$,
	and is recovered post-solve for diagnostic output only.
\item [$\eta$]
...
\item [$G_n$]
	...
	\begin{eqnarray}
		\label{Eq:Tx2}
		G_n &=&  \mathcal{A}_n^x +
		\sum_i [\omega_{in} + \bar{\pi}_{in}]
		+ (0.5\,p_{1n} + 0.85\,p_{2n})
		- e_n
		\nonumber \\
		&& + \sum_i [w_{i1n} + x_{in}]
		\nonumber \\
	...
	The taxable portion $(0.5\,p_{1n} + 0.85\,p_{2n})$ is determined jointly with the
	optimization via the provisional income tier decomposition; see the
	Social Security taxability paragraph in the Constraints section.
...
\item [$\mathbb{G}_n$]
	...
	\begin{equation}
		\mathbb{G}_n = G_n + Q_n + e_n + \sum_i \bar{\zeta}_{in}
		              - (0.5\,p_{1n} + 0.85\,p_{2n}).
	\end{equation}
	Because $G_n$ already contains the taxable Social Security term
	$(0.5\,p_{1n} + 0.85\,p_{2n})$, subtracting it and adding back the full benefit
	$\sum_i\bar{\zeta}_{in}$ restores total Social Security in MAGI,
...
\paragraph*{Taxable ordinary income}
	...
	\begin{eqnarray}
		...
		\mathcal{A}^x_n + \sum_i [\omega_{in} + \bar{\pi}_{in} ]
		\nonumber \\
...
\paragraph*{Social Security taxability}
	...
	\end{equation}
	Expanding $\mathbb{G}_n$ in terms of the LP variables and noting that the taxable
	Social Security term $(0.5\,p_{1n}+0.85\,p_{2n})$ cancels, one finds that $\Pi_n$
	is a purely linear function of the decision variables independent of $p_{rn}$:
	\begin{eqnarray}
		\label{Eq:PIexpand}
		\Pi_n &=& \mathcal{A}^x_n + \mathcal{A}^c_n
		+ \sum_i [\omega_{in} + \bar{\pi}_{in}]
		+ \tfrac{1}{2}\sum_i\bar{\zeta}_{in}
		+ \sum_i [w_{i1n} + x_{in}]
		\nonumber \\
		&& - e_n + Q_n
		+ \sum_i \Bigl[(b_{i0n} - w_{i0n} + d_{in})
		  \bigl(\mu\alpha_{i00n} + \textstyle\sum_{k\neq 0}\alpha_{i0kn}\tau_{kn}\bigr)\Bigr]
		\nonumber \\
		&& - \sum_i \bigl[w_{i0n}\,\alpha_{i00n}\max(0,\tau_{0(n-1)}-\mu)\bigr]
		\nonumber \\
		&& + 0.5\sum_i\Bigl[\kappa_{i0n}
		  \bigl(\mu\alpha_{i00n}+\textstyle\sum_{k\neq 0}\alpha_{i0kn}\tau_{kn}\bigr)\Bigr].
	\end{eqnarray}
	This is structurally identical to the MAGI disaggregation used for Medicare
	(Eq.~\ref{Eq:MedDecomp2}), evaluated at the current year $n$ rather than $n-2$
	and with $\tfrac{1}{2}\bar{\zeta}$ rather than $\bar{\zeta}$ on the right-hand side.

	We partition $\Pi_n$ into three non-negative tier variables $p_{rn}$, $r\in\{0,1,2\}$,
	via the PI decomposition equality
	\begin{equation}
		\label{Eq:C7}
		\sum_{r=0}^{2} p_{rn} = \Pi_n,
	\end{equation}
	with upper bounds
	\begin{equation}
		\label{Eq:PIbounds}
		p_{0n} \le \mathcal{P}^{\mathrm{lo}}_n, \qquad
		p_{1n} \le \mathcal{P}^{\mathrm{hi}}_n - \mathcal{P}^{\mathrm{lo}}_n,
	\end{equation}
	and the 85\% taxability cap
	\begin{equation}
		\label{Eq:TaxableSSCap}
		0.5\,p_{1n} + 0.85\,p_{2n} \le 0.85\sum_i\bar{\zeta}_{in}.
	\end{equation}
	Zone 2 ($r=2$) has no upper bound other than Eq.~(\ref{Eq:TaxableSSCap}).

	The optimizer fills the tiers in the correct order automatically.
	Zone 0 contributes nothing to taxable income;
	zone 1 adds $0.5\,\theta^x_{tn}$ of income tax per dollar of PI above $\mathcal{P}^{\mathrm{lo}}$;
	zone 2 adds $0.85\,\theta^x_{tn}$ per dollar above $\mathcal{P}^{\mathrm{hi}}$.
	Since tax rates $\theta^x_{tn}$ are positive and the optimizer minimizes total taxes,
	PI is allocated to the cheapest available zone first---exactly as the income tax bracket
	variables $f_{tn}$ fill lower-rate brackets before higher-rate ones.
	The upper bound on $p_{1n}$ ensures that once zone 1 is saturated,
	any additional PI spills into zone 2.

	This formulation is closed-form and exact: it replaces the self-consistent
	loop previously used to iterate $\Psi_n$ to convergence, eliminating the
	associated oscillation risk near the $\mathcal{P}^{\mathrm{lo}}$/$\mathcal{P}^{\mathrm{hi}}$
	boundaries.
	Post-solve, the effective fraction is recovered as
	$\Psi_n = (0.5\,p_{1n}+0.85\,p_{2n})/\sum_i\bar{\zeta}_{in}$ for diagnostic output.
...
\paragraph*{Medicare brackets and costs}
	...
	\begin{eqnarray}
		...
		+ \sum_i [\omega_{i(n-2)} + \bar{\zeta}_{i(n-2)} + \bar{\pi}_{i(n-2)}] \nonumber \\
...
\paragraph*{Social Security provisional income tier bounds}
The upper bounds on provisional income tier variables $p_{0n}$ and $p_{1n}$
from Eq.~(\ref{Eq:PIbounds}) add $2N_n$ rows:
\begin{eqnarray}
	A_u[I_8(0, n), q_p(0, n)] &=& 1, \nonumber \\
	u[I_8(0, n)] &=& \mathcal{P}^{\mathrm{lo}}_n, \nonumber \\
	A_u[I_8(1, n), q_p(1, n)] &=& 1, \nonumber \\
	u[I_8(1, n)] &=& \mathcal{P}^{\mathrm{hi}}_n - \mathcal{P}^{\mathrm{lo}}_n, \nonumber \\
	&&\qquad\forall n \in \{0,\ldots, N_n - 1\}.
\end{eqnarray}
The 85\% taxability cap of Eq.~(\ref{Eq:TaxableSSCap}) adds $N_n$ more rows:
\begin{eqnarray}
	A_u[I_8(2, n), q_p(1, n)] &=& 0.5, \nonumber \\
	A_u[I_8(2, n), q_p(2, n)] &=& 0.85, \nonumber \\
	u[I_8(2, n)] &=& 0.85\sum_i\bar{\zeta}_{in}, \nonumber \\
	&&\qquad\forall n \in \{0,\ldots, N_n - 1\}.
\end{eqnarray}
...
\begin{eqnarray}
	...
	+ \sum_i \left[\omega_{i(n-2)} + \bar{\zeta}_{i(n-2)} + \bar{\pi}_{i(n-2)} \right] \nonumber \\
...
The taxable Social Security terms $p_{1n}$ and $p_{2n}$ appear with negative
coefficients on the LHS, reflecting their move from the right-hand side of the
earlier (SC-loop) formulation.
\begin{eqnarray}
	A_e[J_7(n), q_e(n)] &=& 1, \nonumber \\
	A_e[J_7(n), q_{f}(t, n)] &=& 1, \nonumber \\
	A_e[J_7(n), q_p(1, n)] &=& -0.5, \nonumber \\
	A_e[J_7(n), q_p(2, n)] &=& -0.85, \nonumber \\
...
\end{eqnarray}
Note that $\Psi_n\bar\zeta_{in}$ no longer appears on the right-hand side;
it is replaced by the LP variables $p_{1n}$ and $p_{2n}$ on the left.

\paragraph*{Provisional income decomposition}
The PI decomposition equality Eq.~(\ref{Eq:C7}) adds $N_n$ rows to $A_ey = v$.
The structure is identical to the MAGI disaggregation (paragraph above), but
applied at the current year $n$ and with $\tfrac{1}{2}\bar{\zeta}$ rather than
$\bar{\zeta}$ on the right-hand side.
\begin{eqnarray}
	A_e[J_8(n), q_p(r, n)] &=& 1, \quad r \in \{0, 1, 2\}, \nonumber \\
	A_e[J_8(n), q_e(n)] &=& -1, \nonumber \\
	A_e[J_8(n), q_w(i, 1, n)] &=& -1, \nonumber \\
	A_e[J_8(n), q_x(i, n)] &=& -1, \nonumber \\
	A_e[J_8(n), q_b(i, 0, n)] &=&
	    -\mu\alpha_{i00n} - \sum_{k\neq 0}\alpha_{i0kn}\tau_{kn}, \nonumber \\
	A_e[J_8(n), q_d(i, n)] &=&
	    -\mu\alpha_{i00n} - \sum_{k\neq 0}\alpha_{i0kn}\tau_{kn}, \nonumber \\
	A_e[J_8(n), q_w(i, 0, n)] &=&
	    \mu\alpha_{i00n} + \sum_{k\neq 0}\alpha_{i0kn}\tau_{kn}
	    - \alpha_{i00n}\max(0,\tau_{0\max(0,n-1)}-\mu), \nonumber \\
	v[J_8(n)] &=& \mathcal{A}^x_n + \mathcal{A}^c_n
	+ \sum_i \bigl[\omega_{in} + \tfrac{1}{2}\bar{\zeta}_{in} + \bar{\pi}_{in}\bigr]
	\nonumber \\
	&& + 0.5\sum_i\Bigl[\kappa_{i0n}
	   \bigl(\mu\alpha_{i00n}+\textstyle\sum_{k\neq 0}\alpha_{i0kn}\tau_{kn}\bigr)\Bigr],
	\nonumber \\
