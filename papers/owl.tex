...
\item [$q_{pn}$]
	...
	and together partition the total LTCG via $q_{0n} + q_{1n} + q_{2n} \ge Q_n$.
	The bracket widths are enforced by $q_{0n} \le \max(0, T^{15}_n - G_n)$ and
	$q_{0n} + q_{1n} \le \max(0, T^{20}_n - G_n)$, where $T^{15}_n$ and $T^{20}_n$ are the
	inflation-adjusted total taxable income thresholds above which the 15\% and 20\% rates
	apply, and $G_n = \sum_t f_{tn}$ is the ordinary taxable income (evaluated from the
	previous SC iteration). Since the cost function is strictly convex ($0 < 0.15 < 0.20$),
...
\item [$U_n$]
	...
	For reporting purposes, the effective rate $\psi_n = U_n / Q_n$ is computed after
	the solve when $Q_n > 0$; it is not a formulation variable.
...
\paragraph*{LTCG bracket constraints}
	...
	occupied by ordinary income.  Using the value of $G_n$ from the previous SC iteration
	as a parameter (consistent with the treatment of $\Psi_n$, $\mathcal{J}_n$, and
	$\mathcal{M}_n$), the constraints are
	\begin{eqnarray}
		\label{Eq:LTCG2}
...
\begin{eqnarray}
	...
	A_e[J_5(n), q_w(i, j ,n)] &=& -1 + 0.1\,\delta(j,1)\,(1-\mathcal{H}(n-n_{i,59\frac{1}{2}})), \nonumber \\
