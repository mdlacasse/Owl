...
Similarly, the Household Financial Profile currently in effect can be saved back to a workbook with

```python
plan.saveHFP(overwrite=True)
```

which writes `HFP_<plan name>.xlsx` in a format that `readHFP()` can read back. This is most useful
for plans built programmatically, where wages, contributions, and Roth conversions were set from
Python rather than loaded from a workbook.
