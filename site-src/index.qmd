...
or *histogaussian* rates which are
...
Spending profiles are adjusted for inflation, and so are all other indexable quantities. Profiles can be
...
* Fixed rates can also be provided, as well as *histogaussian* rates, which are generated using
the statistical characteristics (means and covariance matrix) of
a selected historical year range.

* Pure *gaussian* rates can also be generated
...
After the first passing, the survivor's own benefit and the survivor benefit are tracked as
separate entitlements and the larger of the two is paid each year, as SSA does. The date at
which the survivor benefit is claimed can be set to as soon as eligible, at the survivor's FRA,
or at a chosen age.
...
* In the current implementation, social security is always taxed at 85%, assuming that your taxable income will be larger than 34 k$ (single) or 44 k$ (married filing jointly).
...
* Medicare Part B and Part D (including IRMAA) are included; Part D base premium is optional and can be disabled if you have other drug coverage.
