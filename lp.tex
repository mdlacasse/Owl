...
\item [$\Gamma{tn}$]
	Bound for Federal income tax bracket. Note that $\Gamma_{(-1)n} := 0$, and therefore
...
\item [$\Delta_{tn}$]
	...
	brackets would need to be adjusted accordingly.
\item [$\theta_{tn}$]
...
\item [$T_n$]
	...
	$F_{tn} = f_{tn}\bar{\Delta_{tn}}$ in the optimization. Given that the rates on
	the tax brackets $\theta_{tn}$ are incressing monotonically, we are then naturally
	optimizing to fill in the lower brackets first.
\item [$U_n$]
...
\paragraph*{Income tax brackets}
	...
	imposing an upper bound on $f$. Since we will use the combined variable
	\begin{equation}
...
\paragraph*{Account balances}
	...
	The initial balances $b_{0}$ are one of the main inputs of this model. They are
