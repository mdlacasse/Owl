...
\item [$\beta_{in}$]
	...
	that can be planned. Therefore, $\beta$ can be positive (e.g., sell a house, inheritance)
...
\paragraph*{Account balances}
	...
	Changes include contributions $\kappa$, distributions and withdrawals $w$, conversions $x$, and growth $\tau$.
...
\paragraph*{Net income}
	...
	\begin{eqnarray}
		...
		+ \sum_{ijk} w_{ijkn} + \sum_i \beta_{in} - \sum_{ik}d_{ikn}
		- T_n - U_n.
	\end{eqnarray}
	Note that big-ticket items $\beta$ can be negative or positive.
	...
	\begin{eqnarray}
		...
		&& + \sum_i [\beta_{in} - .5\mu\psi\kappa_{i00n}].
	\end{eqnarray}
...
\begin{eqnarray}
	A_e[J_2(n), q_g(n)] &=& 1, \nonumber \\
	A_e[J_2(n), q_w(i, j ,k, n)] &=& -1,\nonumber \\
	A_e[J_2(n), q_d(i, ,k, n)] &=& 1, \nonumber \\
	A_e[J_2(n), q_f(t, n)] &=& \bar\Delta_{t n}\theta_{t n}, \nonumber \\
	A_e[J_2(n), q_b(i, 0, 0, n)] &=& \psi\mu, \nonumber \\
	A_e[J_2(n), q_w(i, 0, 0, n)] &=& \psi\max(0, \tau_{0n}), \nonumber \\
	A_e[J_2(n), q_{b^-}(i, 0, 0, n)] &=& \psi\max(0, \tau_{0n}), \nonumber \\
	&&\qquad\forall t \in \{0,\ldots, N_t-1\},\nonumber\\
	&&\qquad\forall i \in \{0,\ldots, N_i-1\},\nonumber\\
	&&\qquad\forall j \in \{0,\ldots, N_j-1\},\nonumber\\
	&&\qquad\forall k \in \{0,\ldots, N_k-1\},\nonumber\\
...
\begin{eqnarray}
	A_e[J_4(n), q_f(t, n)] &=& \bar{\Delta}_{t n}, \nonumber \\
	A_e[J_4(n), q_w(i, 1, k, n)] &=& -1, \nonumber \\
	A_e[J_4(n), q_x(i, k, n)] &=& -1, \nonumber \\
	A_e[J_4(n), q_b(i, 0, k, n)] &=& -(1-\delta(k, 0))\tau_{kn}, \nonumber \\
	&&\qquad\forall t \in \{0,\ldots, N_t-1\},\nonumber\\
	&&\qquad\forall i \in \{0,\ldots, N_i-1\},\nonumber\\
	&&\qquad\forall k \in \{0,\ldots, N_k-1\},\nonumber\\
	&&\qquad\forall n \in \{0,\ldots, N_n-1\}, \nonumber
...
while $A_e$ has $[(N_k+1)N_iN_j + 3)N_n - 1]$ or $33N_n-1$ rows. For a 30-year
...
\begin{eqnarray}
	\label{Eq:Alloc3}
	A[J_5(i, j, k, n), q_b(i, j, k', n)] &=& \delta(k, k') - \alpha_{ijkn}, \nonumber\\
	v[J_5(i, j, k, n)] &=& 0, \\
	&&\qquad\forall i \in \{0,\ldots, N_i-1\},\nonumber\\
	&&\qquad\forall j \in \{0,\ldots, N_j-1\},\nonumber\\
