...
\item [$\kappa_{ijkn}$]
	...
	and that individual selects asset class $k$ when depositing. Most likely, a contribution
...
\item [$G_n$]
	...
	Notice that $G_n$ is also defined by Eq.~(\ref{Eq:Tx1}).
...
\item [$Q_n$]
	...
	to have been purchased more than a year ago and others $b^-$ sold at the end
...
\end{equation}
as the constraints to be enforced. Once $g_0$ is determined, the whole time series is determined as well.
...
\paragraph*{Taxable ordinary income}
	...
	\end{eqnarray}
	and re-arrange to move variables on the LHS as follows
	\begin{eqnarray}
		\label{Eq:C6}
