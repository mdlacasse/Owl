...
| `taxable_cost_basis` | list of `N_i` floats | *(Optional)* Adjusted cost basis of each individual's taxable account (in thousands of dollars). When any entry is positive, Owl tracks unrealized gains using the average-cost method on withdrawals. Omit the field, or use all zeros, to keep the legacy approximation (current-year price appreciation only). Programmatic `setCostBasis([0, …])` still sets an explicit zero basis (100% gain fraction) when needed |
