...
def test_representative_sample_returns_historical():
    """representative_sample should return the historical data pool, not synthetic samples."""
    model = _make_model(frm=1928, to=2025)
    rep = model.representative_sample(10)
    # 1928–2025 inclusive = 98 rows
    assert rep.shape == (98, 4)
    assert np.allclose(rep, model._historical_data)
