...
def test_spia_premium_reduces_tax_deferred_balance():
    ...
    assert bal_spia < bal_base, f"SPIA balance should be lower: {bal_spia:.0f} vs {bal_base:.0f}"
...
def test_spia_income_is_taxable():
    ...
    assert p_spia.e_n[n_buy] > p_base.e_n[n_buy] - 1, "SPIA income should increase taxable income in buy year"
...
def test_spia_past_buy_year_no_premium():
    ...
    assert abs(p.spiaBar_in[0, 0] - 6000) < 200, f"Expected ~$6,000/yr at year 0, got ${p.spiaBar_in[0, 0]:.0f}"
...
def test_spia_indexed_grows_with_inflation():
    ...
    assert p_idx.spiaBar_in[0, n] > p_fixed.spiaBar_in[0, n], (
        f"Indexed SPIA should be larger at year {n}: {p_idx.spiaBar_in[0, n]:.0f} vs {p_fixed.spiaBar_in[0, n]:.0f}"
    )
...
def test_spia_fixed_nominal_constant():
    ...
    assert real_n2 < real_n1 - 1, f"Fixed SPIA real value should decrease: {real_n2:.0f} < {real_n1:.0f}"
...
def test_spia_survivor_fraction_continues_income():
    ...
    p.addSPIA(individual=0, buy_year=buy_year, premium=100_000, monthly_income=500, survivor_fraction=0.5)
    p.solve("maxSpending")
    ...
    assert p.spiaBar_in[0, n_after] == pytest.approx(0.0, abs=1.0), "Annuitant should have no income after death"
    # individual 1 gets survivor benefit
    assert p.spiaBar_in[1, n_after] > 0, "Survivor should continue receiving SPIA income"
...
def test_spia_single_life_no_survivor():
    ...
    p = _couple_plan()
    buy_year = p.year_n[0] + 1
    p.addSPIA(individual=0, buy_year=buy_year, premium=50_000, monthly_income=300, survivor_fraction=0.0)
    p.solve("maxSpending")
...
def test_spia_toml_round_trip():
    ...
    assert abs(p.spiaBar_in[0, 0] - 4800) < 200, f"Expected ~$4,800/yr ($400/mo×12), got ${p.spiaBar_in[0, 0]:.0f}"
...
"""
    ...
    assert p.spia_premiums_in[0, n_buy] == pytest.approx(100_000.0), "Premium should be deducted in buy year"
