...
def _make_plan(name, taxable_k, tax_deferred_k, tax_free_k, rate_year=2000):
    ...
    p.setAccountBalances(
        taxable=[taxable_k], taxDeferred=[tax_deferred_k], taxFree=[tax_free_k],
        startDate="1-1"
    )
...
class TestCostBasisHighGain:
    ...
    def test_higher_q_n_with_high_basis(self):
        ...
        assert total_with_basis > total_no_basis * 3, (
            f"Expected Q_n with basis (total={total_with_basis:,.0f}) to be >> "
            f"no-basis (total={total_no_basis:,.0f})"
        )
