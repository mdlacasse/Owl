...
def test_withdrawal1():
    ...
    options = {"maxRothConversion": 0, "bequest": 0, "withSCLoop": False}
...
def test_taxfreegrowth1():
    ...
    options = {"maxRothConversion": 0, "netSpending": 0, "withSCLoop": False, "xorConstraints": False}
...
def test_taxfreegrowth4():
    ...
    rate = 4
    p.setRates("user", values=[0, 0, rate, 0])
    options = {"maxRothConversion": 0, "netSpending": 0, "withSCLoop": False}
    p.solve("maxBequest", options=options)
    assert p.basis == pytest.approx(0, abs=0.01)
...
def test_Historical1():
    ...
    p.setAccountBalances(taxable=[90.5], taxDeferred=[600.2], taxFree=[50 + 20.6], startDate="1-1")
...
def test_Historical2():
    ...
    p = owl.Plan(inames, dobs, expectancy, name)
    p.setSpendingProfile("smile", 60)
    p.setAccountBalances(taxable=[90.5, 60], taxDeferred=[600.2, 150], taxFree=[50 + 20.6, 40.8], startDate="1-1")
...
def test_fire_roth_ladder():
    ...
    assert np.sum(w_roth[5:n595]) > 10, f"Expected Roth withdrawals after year 5, got {np.sum(w_roth[5:n595]):.1f}"
