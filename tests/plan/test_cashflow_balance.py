...
def _assert_cashflow_balance(p, atol=1.0):
    ...
    """
    lhs = p.g_n + p.s_n + p.T_n + p.st_T_n + p.U_n + p.J_n + p.m_n + p.M_n + p.aca_costs_n + p.debt_payments_n
    rhs = (
    ...
    np.testing.assert_allclose(lhs, rhs, atol=atol, err_msg="Cash flow balance violated")


def _make_single(name, taxable, tax_deferred, tax_free, ss_pia=(0,), ss_age=(67,), rate_year=2000):
...
def _make_couple(name, taxable, tax_deferred, tax_free, ss_pias, ss_ages, rate_year=2000):
    thisyear = date.today().year
    p = owl.Plan(["Alex", "Pat"], [f"{thisyear - 66}-06-01", f"{thisyear - 63}-06-01"], [85, 85], name)
    p.setSpendingProfile("flat", 60)
    p.setAccountBalances(taxable=taxable, taxDeferred=tax_deferred, taxFree=tax_free, startDate="1-1")
    p.setAllocationRatios("individual", generic=[[[60, 40, 0, 0], [70, 30, 0, 0]], [[50, 50, 0, 0], [60, 40, 0, 0]]])
...
class TestCashflowBalance:
    def test_single_roth_only(self):
        ...
        p = _make_single("roth_only", taxable=[0], tax_deferred=[0], tax_free=[1000], ss_pia=[0])
        p.solve("maxSpending")
        _assert_cashflow_balance(p)

    def test_single_tax_deferred(self):
        """Single person, IRA-heavy: RMDs and ordinary income tax in play."""
        p = _make_single("ira_heavy", taxable=[0], tax_deferred=[800], tax_free=[200], ss_pia=[1800], ss_age=[67])
        p.solve("maxSpending")
        _assert_cashflow_balance(p)

    def test_single_mixed_accounts(self):
        """Single person with taxable, IRA, and Roth accounts and SS."""
        p = _make_single("mixed", taxable=[200], tax_deferred=[500], tax_free=[300], ss_pia=[2000], ss_age=[67])
        p.solve("maxSpending")
        _assert_cashflow_balance(p)
    ...
    def test_max_bequest_objective(self):
        ...
        p = _make_single("bequest", taxable=[100], tax_deferred=[500], tax_free=[200], ss_pia=[1800], ss_age=[67])
    ...
    def test_with_hsa(self):
        ...
        p.setSpendingProfile("flat", 60)
        p.setAccountBalances(taxable=[0], taxDeferred=[500], taxFree=[100], hsa=[50], startDate="1-1")
        p.setAllocationRatios("individual", generic=[[[60, 40, 0, 0], [70, 30, 0, 0]]])
    ...
    def test_with_state_tax(self):
        ...
        p = _make_single("state_tax", taxable=[100], tax_deferred=[500], tax_free=[200], ss_pia=[1800], ss_age=[67])
        p.setStateTax("MN")
        p.solve("maxSpending")
