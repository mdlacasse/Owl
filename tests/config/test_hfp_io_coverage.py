"""
Tests for hfp_io.py to improve coverage.

This module tests HFP reading, error handling, and edge cases
...
def test_read_time_horizons_file_error():
    ...
    with pytest.raises(Exception, match="Could not read file"):
        hfp_io.read(buffer, inames, horizons, mylog)


def test_check_columns_missing_column():
    """Test _checkColumns with missing required column."""
    df = pd.DataFrame({"year": [2024, 2025], "wages": [0, 0]})

    with pytest.raises(ValueError, match="missing required column"):
        hfp_io._checkColumns(df, "Joe", ["year", "wages", "contributions"])


def test_check_columns_removes_extra():
    """Test _checkColumns removes extra columns."""
    df = pd.DataFrame({"year": [2024, 2025], "wages": [0, 0], "contributions": [0, 0], "extra_col": [1, 2]})

    result = hfp_io._checkColumns(df, "Joe", ["year", "wages", "contributions"])
    ...
    df = pd.DataFrame(
        {
            ...
            "Roth IRA ctrb": [0, 0],
            "HSA ctrb": [0, 0],
            "Roth conv": [-30000, 0],  # Negative in a past year (n < 0)
            "big-ticket items": [0, 0],
        }
    )
...
def test_condition_house_tables():
    ...
    result = hfp_io._conditionHouseTables(df_dict, mylog)
