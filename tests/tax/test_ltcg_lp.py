...
def test_ltcg_lp_partition_exact():
    ...
    np.testing.assert_allclose(ltcg_sum, total_ltcg, atol=1.0, err_msg="Partition sum does not match Q_n")
...
def test_ltcg_lp_tax_identity():
    ...
    np.testing.assert_allclose(p.U_n, expected_U, atol=0.01, err_msg="U_n does not equal 0.15*q[1] + 0.20*q[2]")
...
class TestLTCGMilp:
    ...
    def _solve_milp(self, extra_options=None):
        ...
        p = _make_single_plan("ltcg_milp", taxable=1000, tax_deferred=500, tax_free=200, rate_year=1997)
    ...
    def test_ltcg_milp_gn_matches_f_sum(self):
        ...
        np.testing.assert_allclose(p.G_n, gn_from_f, atol=1.0, err_msg="G_n does not match sum(f_tn) in MILP mode")
    ...
    def test_ltcg_milp_q0_room_constraint(self):
        ...
        for n in range(p.N_n):
            ...
            assert p.q_pn[0, n] <= room + 1.0, (
                f"Year {n}: q[0]={p.q_pn[0, n]:.1f} exceeds room15={room:.1f} (G_n={p.G_n[n]:.1f}, T15={T15_n:.1f})"
            )
    ...
    def test_ltcg_milp_matches_loop_result(self):
        ...
        p_loop = _make_single_plan("ltcg_loop", taxable=1000, tax_deferred=500, tax_free=200, rate_year=1997)
        ...
        assert rel_diff < 0.01, (
            f"MILP spending {p_milp.g_n[0]:.0f} differs from loop {p_loop.g_n[0]:.0f} by {100 * rel_diff:.2f}%"
        )
...
class TestLTCGScLoopConsistency:
    ...
    def _make_large_taxable_couple(self, name="LTCGConsistency"):
        ...
        p.setAllocationRatios(
            "individual",
            generic=[[[60, 40, 0, 0], [50, 50, 0, 0]], [[60, 40, 0, 0], [50, 50, 0, 0]]],
        )
...
class TestLTCGPartitionBoundRothFile:
    ...
    def test_q_partition_bounded_by_q_n_with_file_conversion(self):
        """q[0,n]+q[1,n]+q[2,n] <= Q_n + tol for all years under a pinned Roth conversion."""
